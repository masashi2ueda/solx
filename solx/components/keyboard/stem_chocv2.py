...
class ChocV2Stem(SolxObject):
    """SolxObject: The keycap with Choc V2 stem attached."""
    def __init__(self, stem_radius = 2.760, inverted=False, stem_height=6.5):
        ...
        Args:
            ...
            stem_height (float): Height of the stem. Default is 6.5mm to ensure it can be properly subtracted from the keycap top.
        ...
        super().__init__(stem_with_slots.node)
        self.cross_slot = cross_slot
        self.stem_radius = stem_radius

    def param_apply(self, func_name: str, params: Vec3) -> ChocV2Stem:
        dst = copy.deepcopy(self)
        dst.cross_slot = param_apply(func_name, params, self.cross_slot)
        return dst
...
if __name__ == "__main__":
    ...
    stem = ChocV2Stem()
    stem.save_stl(f"{dst_dir_path}/chocv2_stem{int(MGN*100)}-{int(sr*100)}.stl")
